...
city_counts = data['市级'].value_counts().head(20)
...
top_20_counties_with_city = county_with_city_counts.sort_values(by='count', ascending=False).head(20)

print("\nTop 20 Counties and their corresponding Cities:")
print(top_20_counties_with_city)

# 获取乡镇级的前20个并输出它们的上级（对应的区县级）
town_with_county = data[['区县级', '乡镇级']].dropna()
town_with_county_counts = town_with_county.groupby(['区县级', '乡镇级']).size().reset_index(name='count')
top_20_towns_with_county = town_with_county_counts.sort_values(by='count', ascending=False).head(20)
...
# # 绘制折线图
# plt.figure(figsize=(20, 6))  # 设置图表的大小
#
# # 映射函数
# def linear_map_below_200(y):
#     """将小于等于200的部分按线性方式映射"""
#     return 40+(y-40)*0.25 # 斜率为1
#
# def linear_map_above_200(y):
#     """将大于200的部分按线性方式映射，斜率不同"""
#     return 200 + ((y-40)*0.25 - 640) * 0.2  # 斜率为0.5
#
# def linear_map_below_40(y):
#     """将小于等于200的部分按线性方式映射"""
#     return y  # 斜率为1
#
# # 应用映射，将原始数据的纵坐标进行变换
# transformed_y = []
# for y in weekly_counts_df['Photo Count']:
#     if 40< y <= 200:
#         transformed_y.append(linear_map_below_200(y))
#     elif y<=40:
#         transformed_y.append(linear_map_below_40(y))
#     elif y>200 :
#         transformed_y.append(linear_map_above_200(y))
#
# # 绘制变换后的数据点
# plt.plot(weekly_counts_df['Week Start'], transformed_y, marker='o', linestyle='-', color='b')
#
# # 设置图表标题和标签
# plt.title('每周拍照数量', fontsize=16)
# plt.xlabel('每周', fontsize=12)
# plt.ylabel('照片数量', fontsize=12)
#
# # 横坐标只显示总的开始和结束日期
...
#
# # 手动设置纵坐标刻度，0-200范围为20，200以上范围为50
# y_ticks = []
# for i in range(0, 41, 5):  # 0-200范围，每隔20
#     y_ticks.append(i)
# for i in range(40, 201, 20):  # 0-200范围，每隔20
#     y_ticks.append(i)
# for i in range(200, max(weekly_counts_df['Photo Count']), 100):  # 200以上范围，每隔50
#     y_ticks.append(i)
#
# # 对纵坐标进行映射，使得0到200和大于200的部分斜率不同
# y_ticks_mapped = []
# for y in y_ticks:
#     if 40< y <= 200:
#         y_ticks_mapped.append(linear_map_below_200(y))
#     elif y<=40:
#         y_ticks_mapped.append(linear_map_below_40(y))
#     elif y>200 :
#         y_ticks_mapped.append(linear_map_above_200(y))
#
# # 设置纵坐标刻度
# plt.yticks(y_ticks_mapped, [str(int(y)) for y in y_ticks])
#
# # 标注最高的10个数据点
# top_10_indices = sorted(range(len(transformed_y)), key=lambda i: transformed_y[i], reverse=True)[:10]
# for idx in top_10_indices:
#     x = weekly_counts_df['Week Start'].iloc[idx]
#     y = transformed_y[idx]
#
#     # 防止标签重叠，设置不同的偏移量
#     offset = 3  # 偏移量，可以根据需要调整
#     plt.text(x , y+offset, f'{int(20*y-1400)}', ha='center', va='bottom',fontweight='bold' ,fontsize=9, color='m')
# # 设置显示网格线
# plt.grid(True)
#
# # 显示图表
